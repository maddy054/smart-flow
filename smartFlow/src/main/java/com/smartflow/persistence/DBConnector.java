...
public class DBConnector implements Connector {
	...
	public void checkPassword(String username , String password) throws OwnException {
		try {
			QueryBuilder queryBuilder = new QueryBuilder(Tables.USER.get());
			String query = queryBuilder.column(2).where(1).buildSelect();
			List<Map<Integer, Object>> result = queryBuilder.executeQuery(query,username);
			
			if(result.isEmpty()) {
				throw new OwnException(ErrorCode.INVALID_USER);
				
			}else if(result.get(0).get(2) != password) {
				throw new OwnException(ErrorCode.WRONG_PASSWORD);
			}
	...
	public List<Sensor> getSensorData(int locationId,int sensorType,int limit,int offset) throws OwnException {
		try {
			...
			for(Map<Integer,Object> map : resultList) {
				...
				sensor.setParameter(SensorType.values()[(int) map.get(3)]);
	...
	public void setSensorData(Sensor sensor) throws OwnException {
		try {
			QueryBuilder  queryBuilder = new QueryBuilder(Tables.SENSOR_DATA.get());
			String query = queryBuilder.buildInsert();
			queryBuilder.execute(query,sensor.getLocationId(),sensor.getTime(),sensor.getParameter().ordinal(),sensor.getValue());
	...
	public String getRecentData(SensorType sensor,int locationId) {
		List<Map<Integer, Object>> result = new ArrayList<>();
		try {
		String query = "SELECT * FROM SENSOR_DATA WHERE PARAMETER = ? AND LOCATION_ID = ? AND TIME = (SELECT MAX(TIME) FROM SENSOR_DATA WHERE PARAMETER = ? )";
		
		QueryBuilder queryBuilder = new QueryBuilder(Tables.LOCATION.get());
		 result = queryBuilder.executeQuery(query, sensor.ordinal(),locationId,sensor.ordinal());
		
		
	}catch(SQLException e) {
		e.printStackTrace();
	}
		return (String) result.get(0).get(4);
	}
}
